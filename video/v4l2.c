...
/* open
 * VIDIOC_QUERYCAP 判断表名是摄像头设备(streaming/read,write)
 * VIDIOC_ENUM_FMT 列举摄像头数据格式
 * VIDIOC_S_FMT    设置数据格式ʽ
 * VIDIOC_REQBUFS 申请buffer
 对于 streaming:
 * VIDIOC_QUERYBUF 申请buffe ， mmap
 * VIDIOC_QBUF     将数据放入队列命令
 * VIDIOC_STREAMON 开启
 * poll           
 * VIDIOC_DQBUF    将数据取出队列
 * 循环....
 * VIDIOC_QBUF     放入队列
 * ....
 对于 read,write:
